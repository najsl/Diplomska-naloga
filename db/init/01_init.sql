...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'fk_accident_weather_observation'
    ) THEN
        ALTER TABLE core.accident
            ADD CONSTRAINT fk_accident_weather_observation
            FOREIGN KEY (weather_observation_id)
